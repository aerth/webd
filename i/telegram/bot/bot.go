	...
	go func() {
		for {
			...
			case <-bot.shutdownChannel:
				close(ch)
				log.Println("Telegram connection closed")
				return
			default:
			}

			if ch == nil {
				log.Println("Telegram connection closed 2")
				return
			}

			updates, err := bot.GetUpdates(config)
			if err != nil {
				log.Println("error fetching telegram updates:", err)
				log.Println("reconnecting to telegram in 3 seconds...")
